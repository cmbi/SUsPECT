...
=head1 SYNOPSIS
 ...
 ./vep -i variations.vcf --plugin TranscriptAnnotator,/path/to/annotation.txt

=head1 DESCRIPTION

 A VEP plugin that annotates transcript consequences in a VCF file based on
 a given file:
     
   --plugin TranscriptAnnotator,${HOME}/file.txt.gz
 
 To create/use a custom GFF file, these programs must be installed in your path:
   * The tabix and bgzip utilities to read the annotation: check
...
sub get_header_info {
  return {
      PPH2_score => 'PolyPhen-2 score',
      PPH2_prediction => 'PolyPhen-2 prediction'
  };
}
...
sub parse_data {
  ...
  return {
    ...
    result => {
      PPH2_score => _trim($score),
      PPH2_prediction => _trim($pred)
    }
  };
}
